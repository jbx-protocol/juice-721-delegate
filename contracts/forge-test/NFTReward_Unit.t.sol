...
contract TestJBTieredNFTRewardDelegate is Test {
  ...
  function testJBTieredNFTRewardDelegate_tiers_returnsAllTiers(uint256 numberOfTiers) public {
    ...
    for (uint256 i; i < numberOfTiers; i++) {
      ...
      _tiers[i] = JB721Tier({
        ...
        royaltyRate: _tierParams[i].royaltyRate,
        royaltyBeneficiary: _tierParams[i].royaltyBeneficiary,
    ...
    for (uint256 i; i < numberOfTiers; i++) {
      _delegate.test_store().ForTest_setTier(
        ...
        JBStored721Tier({
          ...
          royaltyRate: uint8(_tierParams[i].royaltyRate),
          category: uint8(_tierParams[i].category), 
          allowManualMint: false,
          transfersPausable: false
        })
      );
  ...
  function testJBTieredNFTRewardDelegate_use_default_reserved_token_beneficiary() public {
    ...
    assertEq(_delegate.test_store().defaultRoyaltyBeneficiaryOf(address(_delegate)), reserveBeneficiary);
  }
  ...
  function testJBTieredNFTRewardDelegate_mintFor_mintArrayOfMintForTiersData() public {
    uint256 nbTiers = 3;

    address beneficiaryTwo = address(bytes20(keccak256('beneficiaryTwo')));
    ...
    JB721TierParams[] memory _tiers = new JB721TierParams[](nbTiers);
    JBTiered721MintForTiersData[] memory _tiersToMint = new JBTiered721MintForTiersData[](2);
    uint16[] memory _tiersToMintOne = new uint16[](nbTiers * 2);
    uint16[] memory _tiersToMintTwo = new uint16[](nbTiers * 2);
    ...
    for (uint256 i; i < nbTiers; i++) {
      ...
      _tiersToMintOne[_tiersToMintOne.length - 1 - i] = uint16(i)+1;

      _tiersToMintTwo[i] = uint16(i)+1;
      _tiersToMintTwo[_tiersToMintTwo.length - 1 - i] = uint16(i)+1;
    }

    _tiersToMint[0] = JBTiered721MintForTiersData({ tierIds: _tiersToMintOne, beneficiary: beneficiary});
    _tiersToMint[1] = JBTiered721MintForTiersData({ tierIds: _tiersToMintTwo, beneficiary: beneficiaryTwo});
    ...
    vm.prank(owner);
    _delegate.mintFor(_tiersToMint);

    assertEq(_delegate.balanceOf(beneficiary), 6);
    assertEq(_delegate.balanceOf(beneficiaryTwo), 6);
    ...
    assertEq(_delegate.ownerOf(_generateTokenId(1, 3)), beneficiaryTwo);
    assertEq(_delegate.ownerOf(_generateTokenId(1, 4)), beneficiaryTwo);
    assertEq(_delegate.ownerOf(_generateTokenId(2, 3)), beneficiaryTwo);
    assertEq(_delegate.ownerOf(_generateTokenId(2, 4)), beneficiaryTwo);
    assertEq(_delegate.ownerOf(_generateTokenId(3, 3)), beneficiaryTwo);
  ...
  function testJBTieredNFTRewardDelegate_tiers_adjustTier_remove_tiers_multiple_times(
    ...
    for (uint256 i; i < floorTiersToAdd.length; i++) {
      ...
      _tiersAdded[i] = JB721Tier({
        ...
        royaltyRate: _tierParamsToAdd[i].royaltyRate,
        royaltyBeneficiary: _tierParamsToAdd[i].royaltyBeneficiary,
    ...
    _storedTiers = _delegate.store().tiers(
      address(_delegate),
      0,
      0,
  ...
  function testJBTieredNFTRewardDelegate_adjustTiers_with_same_category_multiple_times(
    ...
    JB721Tier[] memory _stored100Tiers = _delegate.store().tiers(
      address(_delegate),
      100,
      0,
  ...
  function testJBTieredNFTRewardDelegate_adjustTiers_with_different_categories(
    ...
    JB721Tier[] memory _allStoredTiers = _delegate.store().tiers(
      address(_delegate),
      0,
      0,
      initialNumberOfTiers + floorTiersToAdd.length * 2
    ...
    JB721Tier[] memory _stored102Tiers = _delegate.store().tiers(
      address(_delegate),
      102,
      0,
    ...
    for (uint256 i; i < initialNumberOfTiers; i++) {
      ...
      if (i + 1 != 1 && i + 1 != 3 && i + 1 != 4) {
        ...
        _tiersRemaining[_arrayIndex] = JB721Tier({
          ...
          royaltyRate: _tierDataRemaining[_arrayIndex].royaltyRate,
          royaltyBeneficiary: _tierDataRemaining[_arrayIndex].royaltyBeneficiary,
  ...
  function testJBTieredNFTRewardDelegate_adjustTiers_revertIfAddingWithReservedRate(
    ...
    for (uint256 i; i < initialNumberOfTiers; i++) {
      ...
      _tiers[i] = JB721Tier({
        ...
        royaltyRate: _tierParam[i].royaltyRate,
        royaltyBeneficiary: _tierParam[i].royaltyBeneficiary,
